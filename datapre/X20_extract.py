...
# 后面要使用交叉验证 每个数据都会做train和val，所以提取的时候也要提取其train和val两种
...
class PatchDataset(data_utils.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pid = osp.basename(osp.dirname(osp.dirname(img_fp)))
        sub_pid = osp.basename(osp.dirname(img_fp))
        img_bname = osp.basename(img_fp).rsplit('.', 1)[0]
        aug_img = self.trans(image=img)['image']
        return pid, sub_pid, img_bname, aug_img
...
def save_val_feat_in_thread(batch_pid, batch_subpid, batch_img_bname, batch_val_feat, save_dir):
    for b_idx, (pid, sub_pid, img_bname, val_feat) in enumerate(zip(batch_pid, batch_subpid, batch_img_bname, batch_val_feat)):
        feat_save_dir = osp.join(save_dir, pid,sub_pid)
        os.makedirs(feat_save_dir, exist_ok=True)
        feat_save_name = osp.join(feat_save_dir, f'{img_bname}.pkl')
        if osp.exists(feat_save_name):
            with open(feat_save_name, 'rb') as infile:
...
def pred_and_save_with_dataloader(model, img_fp_list, save_dir, batch_size=1, dataset_class="imagenet"):
    ...
    for batch in progress.track(val_dl):
        batch_pid, batch_subpid, batch_img_bname, batch_tr_img = batch
        batch_tr_img = batch_tr_img.cuda()
        with torch.no_grad():
            val_feat = model(batch_tr_img)
            val_feat = val_feat.cpu().numpy()
            # 启用多线程来保存数据
            executor.submit(save_val_feat_in_thread, batch_pid, batch_subpid, batch_img_bname, val_feat, save_dir)
    print("task finished..")
...
class EffNet(nn.Module):
    def __init__(self, efname='efficientnet_v2_s', model_path=""):
        ...
        # print('Use pretrained weight in ImageNet')
        # print(self.model)
        # print(f'Load pretrain model from {model_path}')
        # self.model.load_state_dict(torch.load(model_path),strict=False)
